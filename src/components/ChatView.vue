<template>
    ...
    <ul role="list" class="space-y-6">
        <li v-for="(activityItem, activityItemIdx) in activity" :key="activityItem.id" class="relative flex gap-x-4">
            ...
            <template v-if="activityItem.type === 'user-request'">
                <img
                    :src="activityItem.person.imageUrl"
                    alt=""
                    class="relative mt-3 size-6 flex-none rounded-full bg-gray-50"
                    @click="loadSuggest(activityItem.id)"
                />

                <div @click="loadSuggest(activityItem.id)" class="flex-auto rounded-md p-3 ring-1 ring-inset ring-gray-200">
                    <div class="flex justify-between gap-x-4">
                        <div class="py-0.5 text-xs/5 text-gray-500"><span class="font-medium text-slate-900">{{ activityItem.person.name }}</span> user-request</div>
...
const loadSuggest = (_id) => {
    console.log('LOADING SUGGEST for', _id)
